...
class Q1WindowTransformer:
    ...
    def transform(self, pbp_events: List[PbpEventRow], source_url: str) -> Optional[Q1WindowRow]:
        ...
        logger.debug("Processing Q1 window analytics", game_id=game_id, total_events=len(pbp_events))
        
        # Filter to Q1 events within our window
        window_events = []
        for e in pbp_events:
            if e.period == 1 and e.seconds_elapsed is not None:
                # Ensure seconds_elapsed is a number for comparison
                try:
                    if isinstance(e.seconds_elapsed, str):
                        seconds_elapsed = float(e.seconds_elapsed)
                    else:
                        seconds_elapsed = e.seconds_elapsed
                    ...
                    logger.warning(f"Invalid seconds_elapsed value: {e.seconds_elapsed} (type: {type(e.seconds_elapsed)}) for game {game_id}")
                    continue
        ...
        team_stats = {
            home_team: TeamStats(home_team),
            away_team: TeamStats(away_team)
        }
        ...
        pace48_actual = (possessions_elapsed / window_duration_min) * 48.0 if window_duration_min > 0 else None
        
        # Calculate rebound percentages (requires both teams' data)
        home_stats = team_stats[home_team]
        away_stats = team_stats[away_team]
        
        home_orb_pct = self._calculate_orb_pct(home_stats.offensive_rebounds, away_stats.defensive_rebounds)
        home_drb_pct = self._calculate_drb_pct(home_stats.defensive_rebounds, away_stats.offensive_rebounds)
        away_orb_pct = self._calculate_orb_pct(away_stats.offensive_rebounds, home_stats.defensive_rebounds)
        away_drb_pct = self._calculate_drb_pct(away_stats.defensive_rebounds, home_stats.offensive_rebounds)
    ...
    def _process_window_events(self, events: List[PbpEventRow], team_stats: Dict[str, TeamStats]) -> int:
        ...
        for event in events:
            ...
            # Track fouls and bonus situations - assign to the fouling team
                ...
                if stats.team_fouls_in_quarter >= 4 and stats.bonus_start_time is None:
                    # Ensure seconds_elapsed is a number for bonus time calculation
                    try:
                        if isinstance(event.seconds_elapsed, str):
                            stats.bonus_start_time = float(event.seconds_elapsed)
                        else:
                            stats.bonus_start_time = event.seconds_elapsed
                    except (ValueError, TypeError):
                        # Skip setting bonus time if invalid
                        pass

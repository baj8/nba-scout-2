...
def extract_game_meta(summary_resp: dict, boxscore_resp: dict) -> dict:
    ...
    if not game_id:
        raise ValueError(
            "Missing game_id in both summary and boxscore parameters. Switch to extract_game_meta(summary_resp, boxscore_resp) for better reliability."
        )
    ...
    try:
        ...
        home_team_id_int = to_int_or_none(home_team_id)
        away_team_id_int = to_int_or_none(away_team_id)

        if home_team_id_int is None or away_team_id_int is None:
            raise ValueError(f"Invalid team IDs: home={home_team_id}, away={away_team_id}")

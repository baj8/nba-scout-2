...
def extract_game_outcomes(
    ...
    try:
        ...
        logger.debug("Extracted game outcomes", 
                    game_id=game_id,
        ...
        logger.error("Failed to extract game outcomes", 
    ...
    try:
        ...
        for player_data in home_players:
            try:
                ...
                logger.warning("Failed to create home lineup row",
                              game_id=game_id, player=player_data.get('player'), error=str(e))
        ...
        for player_data in away_players:
            try:
                ...
                logger.warning("Failed to create away lineup row",
                              game_id=game_id, player=player_data.get('player'), error=str(e))
        
        logger.info("Extracted starting lineups from B-Ref",
                   game_id=game_id, count=len(lineups))
        
    except Exception as e:
        logger.error("Failed to extract starting lineups",
                    game_id=game_id, error=str(e))
    ...
    try:
        ...
        for injury_info in injury_data:
            try:
                ...
                logger.warning("Failed to create injury status row",
                              game_id=game_id, 
                              player=injury_info.get('player'), 
                              error=str(e))
        
        logger.info("Extracted injury notes from B-Ref",
                   game_id=game_id, count=len(injury_rows))
        
    except Exception as e:
        logger.error("Failed to extract injury notes",
                    game_id=game_id, error=str(e))
...
def extract_game_metadata(
    ...
    try:
        ...
        logger.debug("Extracted game metadata", 
                    game_id=game_id, metadata=metadata)
        
    except Exception as e:
        logger.warning("Failed to extract game metadata",
                      game_id=game_id, error=str(e))

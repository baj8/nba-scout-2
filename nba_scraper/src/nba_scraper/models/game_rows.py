...
class GameRow(BaseModel):
    ...
    def validate_game_data(self) -> 'GameRow':
        ...
        # Ensure game_date_local matches arena timezone
        if self.game_date_utc and self.arena_tz:
            arena_tz_obj = tz.gettz(self.arena_tz)
            local_dt = self.game_date_utc.astimezone(arena_tz_obj)
            expected_local_date = local_dt.date()
            
            if self.game_date_local != expected_local_date:
                # Auto-correct the local date
                self.game_date_local = expected_local_date
    ...
    def from_nba_stats(cls, data: Dict[str, Any], source_url: str) -> 'GameRow':
        ...
        home_team = (
            data.get('HOME_TEAM_ABBREVIATION') or 
            data.get('HOME_TEAM_TRICODE') or
            data.get('homeTeam', {}).get('teamTricode', '') if isinstance(data.get('homeTeam'), dict) else ''
        )
        
        away_team = (
            data.get('VISITOR_TEAM_ABBREVIATION') or 
            data.get('AWAY_TEAM_ABBREVIATION') or
            data.get('VISITOR_TEAM_TRICODE') or
            data.get('AWAY_TEAM_TRICODE') or
            data.get('awayTeam', {}).get('teamTricode', '') if isinstance(data.get('awayTeam'), dict) else ''
        )
        ...
        if not home_team:
            ...
            if isinstance(home_obj, dict):
                home_team = (
                    home_obj.get('abbreviation') or
                    home_obj.get('tricode') or
                    home_obj.get('teamTricode') or
                    ''
                )
        ...
        if not away_team:
            ...
            if isinstance(away_obj, dict):
                away_team = (
                    away_obj.get('abbreviation') or
                    away_obj.get('tricode') or
                    away_obj.get('teamTricode') or
                    ''
                )

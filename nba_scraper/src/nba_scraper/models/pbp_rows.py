...
class PbpEventRow(BaseModel):
    ...
    @classmethod
    def enrich_with_shot_chart(
        cls, pbp_row: "PbpEventRow", shot_data: Dict[str, Any]
    ) -> "PbpEventRow":
        ...
        shot_zone = cls._classify_shot_zone(
            pbp_row.shot_x, pbp_row.shot_y, pbp_row.shot_distance_ft
        )
    ...
    def _classify_shot_zone(
        x: Optional[float], y: Optional[float], distance: Optional[float]
    ) -> Optional[ShotZone]:
        """Classify shot zone based on coordinates."""
        if distance is None:
            return None

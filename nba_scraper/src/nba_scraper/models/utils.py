...
def preprocess_nba_stats_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    enum_fields = {
        # Game status fields
        'GAME_STATUS_TEXT', 'status', 'STATUS', 'game_status', 'GAME_STATUS',
        
        # Event type fields (now handled by preprocess_pbp_event_data, but keeping for safety)
        'EVENT_TYPE', 'event_type', 'EVENTMSGTYPE', 'EVENTMSGACTIONTYPE',
        'event_msg_type', 'EVENT_MSG_TYPE', 'eventmsgtype', 'eventType',
        
        # Injury/player status fields
        'INJURY_STATUS', 'injury_status', 'player_status', 'PLAYER_STATUS',
        'availability_status', 'AVAILABILITY_STATUS',
        
        # Referee role fields
        'REFEREE_ROLE', 'referee_role', 'role', 'ROLE', 'ref_role', 'REF_ROLE',
        'official_role', 'OFFICIAL_ROLE',
        
        # Position fields (now handled by preprocess_pbp_event_data, but keeping for safety)
        'POSITION', 'position', 'POS', 'pos', 'player_position', 'PLAYER_POSITION',
        'START_POSITION', 'start_position',
        
        # Shot/event related enums (now handled by preprocess_pbp_event_data, but keeping for safety)
        'SHOT_TYPE', 'shot_type', 'shotType', 'SHOT_RESULT', 'shot_result',
        'SHOT_ZONE', 'shot_zone', 'shotZone', 'SHOT_AREA', 'shot_area',
        'SHOT_ZONE_BASIC', 'shot_zone_basic', 'SHOT_ZONE_AREA', 'shot_zone_area',
        'FOUL_TYPE', 'foul_type', 'foulType', 'FOUL_KIND', 'foul_kind',
        'REBOUND_TYPE', 'rebound_type', 'reboundType', 'REB_TYPE', 'reb_type',
        'TURNOVER_TYPE', 'turnover_type', 'turnoverType', 'TO_TYPE', 'to_type',
        'TIMEOUT_TYPE', 'timeout_type', 'timeoutType',
        
        # Team/game identifiers that might be used in enum comparisons
        'TEAM_TRICODE', 'team_tricode', 'TEAM_CODE', 'team_code',
        'HOME_TEAM_TRICODE', 'home_team_tricode', 'AWAY_TEAM_TRICODE', 'away_team_tricode',
        'HOME_TEAM_ABBREVIATION', 'VISITOR_TEAM_ABBREVIATION', 'AWAY_TEAM_ABBREVIATION',
        
        # Any field ending with these suffixes (common enum patterns)
        '_TYPE', '_STATUS', '_ROLE', '_RESULT', '_ZONE', '_KIND', '_CODE'
    }
    
    # Convert all enum fields to strings
    for key, value in processed_data.items():
        if value is not None:
            ...
            elif any(key.endswith(suffix) for suffix in ['_TYPE', '_STATUS', '_ROLE', '_RESULT', '_ZONE', '_KIND', '_CODE']):
                processed_data[key] = str(value)
    ...
    numeric_enum_fields = {
        'EVENTMSGTYPE', 'EVENTMSGACTIONTYPE', 'GAME_STATUS_ID', 'STATUS_ID',
        'PERIOD_TYPE', 'SHOT_TYPE_ID', 'ACTION_TYPE', 'EVENT_MSG_TYPE', 
        'EVENT_MSG_ACTION_TYPE', 'ACTIONTYPE'
    }
...
def _is_pbp_event_data(data: Dict[str, Any]) -> bool:
    ...
    pbp_indicators = [
        'EVENTMSGTYPE', 'EVENT_MSG_TYPE', 'EVENTMSGACTIONTYPE', 'EVENT_MSG_ACTION_TYPE',
        'GAME_EVENT_ID', 'EVENTNUM', 'event_idx', 'period', 'PERIOD'
    ]

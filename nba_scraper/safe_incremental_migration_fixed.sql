-- Safe Incremental Migration for NBA Stats Foundation (FIXED)
...
-- =============================================================================
-- 2) CREATE MISSING TABLES (FIXED - no generated column)
-- =============================================================================

-- Create lineup_stints table without generated column to avoid immutability issues
CREATE TABLE IF NOT EXISTS lineup_stints (
    ...
    lineup_hash TEXT, -- Regular column, populated by application code
...
-- Lineup stints uniqueness (using lineup_hash instead of generated column)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
CREATE INDEX IF NOT EXISTS idx_ls_lineup_hash 
...
COMMENT ON TABLE lineup_stints IS 'Lineup stints with array-based player tracking and hash for uniqueness';
COMMENT ON COLUMN lineup_stints.lineup_player_ids IS 'Array of 5 player IDs in the lineup';
COMMENT ON COLUMN lineup_stints.lineup_hash IS 'MD5 hash of lineup for uniqueness (populated by application)';
...
-- Show tables count
SELECT 'tables ready' AS status, COUNT(*) AS table_count FROM information_schema.tables WHERE table_schema = 'public';
